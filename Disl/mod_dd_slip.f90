!*==mod_dd_slip.spg  processed by SPAG 6.70Rc at 12:39 on 29 Oct 2015
...
!    Nculeation common block
      ...
      DOUBLE PRECISION :: BDRAG , xle , d , blen , tobs , dstar , &
     &                    VCUTOFF , tref , htinc , epsx , epss
...
!	! --- Gradient Direct terms
         DOUBLE COMPLEX :: ALPHA1G , ALPHA2G , BIALPHAG , BETA1G , &
     &                     BETA2G , BIBETAG
         DOUBLE COMPLEX :: ALPHAD1 , BETAD1 , ALPHAD2 , BETAD2
         DOUBLE COMPLEX :: BETA , BETA1 , BETA2 , BETA3 , BETA4 , &
     &                     BETA5 , BETA6 , BETA7
         DOUBLE COMPLEX :: BIALPHA , BIBETA , PHI1 , PHI2 , PHI11 , &
     &                     PHID1
!>    Multipole terms
!	! --- Gradient terms
...
!$$$                        if (locphi(islp) == 1) then
!$$$                           dlengthslp = lnuc + 100.0d0 + rand(1)*lnu
!$$$                        else
!$$$                           dlengthslp = 2.d0*lnuc + 100.0d0 + rand(1
!$$$     $                          *lnuc
!$$$                        end if
!$$$                     else
!$$$                        dlengthslp = 4.d0*lnuc + 100.0d0 + rand(1)*l
!$$$                     end if
!$$$                     if (locphi(islp) == 1) then
!$$$                        if (yendslp(islp) < 0.0d0) then
!$$$                           cycle
...
!$$$                           rnuc(1,k,islp) = xstart - dlengthslp*cphi
!$$$                        else
!$$$                           rnuc(1,k,islp) = xstart+abs(dlengthslp*cp
!$$$                        endif
!$$$                        rnuc(2,k,islp) = -(ystart + dlengthslp*sphi)
!$$$                        rnuc(3,k,islp) = 0.0
!$$$                     endif
!$$$                     elem_source(k,islp) = fe_locate(rnuc(:,k,islp),
!$$$                     write( *,fmt='(A10,1X,3(1x,E15.7))') 'Coords ar
!$$$     $                    snuc(k,islp),rnuc(1,k,islp), rnuc(2,k,islp
!$$$                     print *, 'Located in Element ', elem_source(k,i
!$$$                     print *, 'Islp = ', islp, locphi(islp), cphi, s
!$$$c$$$                     exit
!$$$                  endif
!$$$            else
!$$$               goto 10
